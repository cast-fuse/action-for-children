...
const timesOfDay = ["afternoon", "evening"];
...
const getVisibleButton = (timeOfDay) =>
  document.querySelector(`[${timeOfDay}]`);
const getRadioInput = (timeOfDay) =>
  document.querySelector(`[${timeOfDay}-radio]`);
...
const clickHiddenRadio = (radio) => (radio ? radio.click() : null);
...
const handleTimeOptions = () => {
  ...
  timesOfDay
    ...
    .map((timePair) =>
      timePair ? updateActiveClasses(timeButtons, timePair.button) : null
    );
  ...
  daysOfWeek
    ...
    .map((dayPair) =>
      dayPair ? updateActiveClasses(dayButtons, dayPair.button) : null
    );
};
